...
if __name__ == "__main__":
    # 示例1: 通过列名（字母）读取
    testfilepath = pathlib.Path(__file__).parent / "test_data" / "3班名单.xlsx"
    data_a = read_excel_column(testfilepath, "Sheet1", column_name="B")
    print(f"A列数据: {data_a}")
    
    # 示例2: 通过列标题读取
    data_name = read_excel_column(testfilepath, "Sheet1", column_name="姓名")
    print(f"姓名列数据: {data_name}")
    
    # 示例3: 通过列索引读取
    data_index = read_excel_column(testfilepath, "Sheet1", column_index=1)
